...
[!IMPORTANT] Make sure that the Model class implements `Eventable`. Without it we are not able to map your model into a calendar event.

For more information, please refer to the Calendar Events section.
...
The `CalendarEvent` takes all available options like the underlying calendar package, for more info read here.
...
1. As a key-value pair where the key is the CSS property and value is the condition under which the style should apply.
2. As a key-value pair where the key is the CSS property and the value is directly the CSS value.
3. As a single string for static styles that always apply.
...
[!IMPORTANT] Make sure that the Model class implements `Resourceable`. Without it, we are not able to map your model into a calendar resource.

For more information, please refer to the Calendar Resources section.
...
Please refer to the Calendar Event: linking to resources section to learn how to do it.
...
The `CalendarResource` takes all available options like the underlying calendar package, for more info read here.
...
[!CAUTION] While we still recommend setting this property to true, it does come with some potential side effects that you need to keep in mind. Keep reading to learn more.
...
To learn more about the FilamentTimezone setting, please refer to the Filament documentation.
...
[!CAUTION] Actions have no default authorization. This means, anyone can use any action.

Please check the Authorization section to learn how to authorize actions.
...
Every action you use in the calendar should be defined as a public method in the widget class, just as usual when adding an Action to a Livewire component in filament.
...
1. Stick to auto discovery, in which case you are ready to go :-)
2. If your calendar works with a single model only or you reuse the same schema for multiple models, you can implement the `schema` or `defaultSchema` method
3. Or you can implement a schema per model
...
1. Define a method with any name you want and add the `#[CalendarSchema()]` attribute,
2. Or define a method in the format `camelCaseModelNameSchema`, such as `fooBarSchema`
...
However, each resource is wrapped in an alpine component, which exposes the resource data that you can freely use using AlpineJS.
...
For detailed information, please follow the default filament documentation on how to add authorization to actions.
